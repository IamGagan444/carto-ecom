...
import { useGetProdoctsCategorieQuery } from "../../redux/ProductApi";
...
const MenProducts = () => {
  ...
  const { selected_category } = useParams();

  const { isLoading, data } = useGetProdoctsCategorieQuery(selected_category, {
  ...
  return (
    <div className="pt-[4rem] cu500:pt-[4.4rem]">
      <div className="text-xl sm:text-2xl md:text-3xl m-6 font-bold flex items-center capitalize">
        <h1 className="text-slate-700">{selected_category} </h1>{" "}
        <AiOutlineSwap className="text-2xl mx-1" />
                ...
                <PopoverHandler {...triggers2}>
                  ...
                  >
                    ...
                    {selectedSort === 1
                      ...
                      : ""}{" "}
                    <FiChevronDown className="text-2xl rot-main " />
                  </Button>
                </PopoverHandler>

                <PopoverContent
                  {...triggers2}
                  className="min-w-[250px] p-0 m-0 "
                >
                  {[1, 2, 3, 4, 5].map((val, i) => (
                    <label
